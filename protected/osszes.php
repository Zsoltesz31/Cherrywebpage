...
<?php
$query="SELECT gyumolcs_tip,hely,terulet,fajta,szedes_mod,raklap.rekesz_fajta,SUM(raklap.rekesz_db) as DB, SUM(raklap.suly) as suly, datum FROM szallitas JOIN raklap ON (szallitas.id = raklap.szallitas_id) GROUP BY hely,gyumolcs_tip,terulet,fajta,rekesz_fajta,szedes_mod,datum";
...
<?php else: ?>									
<div class="lista">
<table class="table-sortable tabla3" id="myTable" style="margin-top:30px;">
  <thead>
    <tr>
...
//gyumolcs
  ...
  for (let indexer = 0; indexer < tr.length; indexer++) {
    ...
    let elem = moment(tr[indexer].getElementsByTagName("td")[8].innerText,"YYYY/MM/DD");
...
fajtateruletek['Lilike']=['','Hármastarján'];
  ...
  let teruletText=teruletek.options[teruletek.selectedIndex].text;
